...
#import sys
#sys.path.append('/content/drive/MyDrive/PRIMER/centrum/A')
#import pretraining_dataset_construction
from pretraining_dataset_construction import truncate_doc
...
class DataTrainingArguments:
    ...
    prediction_mode: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The mode for which the predict api is called"
            )
        },
    )
    ...
    is_pretrain: bool = field(
        default=False,
        metadata={
            "help": "Whether the model is being pretrained"
        }
    )
    ...
    early_stopping_patience: int = field(
        default=None,
        metadata={
            "help": "Number of checkpoints to wait for earlystopping"
        }
    )
...
def main():
    ...
    def dataset_specific_processing(example, examples, example_index):
        if data_args.dataset_name == "multi_news" or "wcep_dataset_loader" in data_args.dataset_name \
                or "wikisum_dataset_loader" in data_args.dataset_name:
            documents = example.split("|||||")[:-1]
            truncated_docs = truncate_doc(max_length=4096, documents=documents, idx=example_index)
            return f"{DOC_SEP_}".join(truncated_docs) + f"{DOC_SEP_}"
        return example
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] is not None and examples[summary_column][i] is not None:
                processed_input = dataset_specific_processing(examples[text_column][i], examples, i)
    ...
    if training_args.do_predict:
        ...
        predict_dataset = raw_datasets[data_args.prediction_mode]
    ...
    # Metric
    metric = load_metric("rouge", experiment_id=random_seed)
    ...
    trainer = Seq2SeqTrainer(
        ...
        callbacks=[EarlyStoppingCallback(data_args.early_stopping_patience, 0.0)]
    )
    ...
    if training_args.do_predict:
        ...
        output_prediction_file = training_args.output_dir
        ...
        with open("20news_generated.txt", "w") as files:
            files.write("\n".join(predictions))
